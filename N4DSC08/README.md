...
- RS485 transceiver is a **CS48520S** manufactured by [ChipAnalog]([CS48520S-Chipanalog](https://e.chipanalog.com/products/interface/interface/interface3/1178))
...
- The 4-pads in the square are used at factory for the MCU programming and the pinout (starting from the square pad, followed by 3 rounded pads) is: GND, SWC (pin3), SWD (pin2), 5V
...
Registers have always a length of 2bytes unless specified
...
- If the value from register is -32768 (0x8000) means sensor is not connected or sensor is broken/don't communicate. See Troubleshooting paragraph
...
| 243 | F3  | 0       |           | Write 1 here if sensor wires are longer then 5 meters |
| 244 | F4  | 10      |           | ?                                                     |
| 245 | F5  | 0       |           | ?                                                     |
| 252 | FC  | 0       |           | ?                                                     |
...
- As you can see, temperature value from probes is duplicated in Holding Registers starting from address `4 0160` (`4x 00A0`)
